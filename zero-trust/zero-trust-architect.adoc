...
Nella terminologia del modello Zero Trust, la classificazione dei dati implica l'identificazione dei dati tossici. I dati tossici sono dati sensibili che non devono essere divulgati al di fuori di un'organizzazione. La divulgazione di dati tossici potrebbe violare la conformità normativa e danneggiare la reputazione di un'organizzazione. In termini di conformità normativa, i dati tossici includono i dati dei titolari di carte per la  https://www.netapp.com/pdf.html?item=/media/17180-tr4401.pdf["Payment Card Industry Data Security Standard (PCI-DSS)"^], i dati personali per l'UE  https://www.netapp.com/responsibility/trust-center/data-privacy-principles-compliance/gdpr-compliance/["Regolamento generale sulla protezione dei dati (GDPR)"^], o i dati sanitari per la  https://www.hhs.gov/hipaa/for-professionals/privacy/laws-regulations/index.html["Health Insurance Portability and Accountability Act (HIPAA)"^]. Puoi usare NetApp https://www.netapp.com/data-services/classification/["NetApp Data Classification"^] (precedentemente noto come Cloud Data Sense), un toolkit basato sull'intelligenza artificiale, per scansionare, analizzare e categorizzare automaticamente i tuoi dati.
...
Oltre al controllo degli accessi basato sui ruoli (RBAC) per l'amministrazione del cluster, https://www.netapp.com/pdf.html?item=/media/17055-tr4647.pdf["Autenticazione multifattore (MFA)"^] può essere implementata per l'accesso amministrativo web a ONTAP e per l'accesso tramite riga di comando Secure Shell (SSH). L'MFA per l'accesso amministrativo è un requisito per le organizzazioni del settore pubblico statunitense o per chi deve seguire il PCI-DSS. L'MFA rende impossibile per un attaccante compromettere un account usando solo nome utente e password. L'MFA richiede due o più fattori indipendenti per autenticarsi. Un esempio di autenticazione a due fattori è qualcosa che l'utente possiede, come una chiave privata, e qualcosa che l'utente conosce, come una password. L'accesso amministrativo web a ONTAP System Manager o Active IQ Unified Manager è abilitato tramite Security Assertion Markup Language (SAML) 2.0. L'accesso tramite riga di comando SSH utilizza un'autenticazione a due fattori concatenata con chiave pubblica e password.
...
Ogni giorno emergono nuovi requisiti per mitigare i rischi dei sistemi di storage e le lacune infrastrutturali quando un'organizzazione riutilizza le unità, restituisce unità difettose o le aggiorna a unità più grandi vendendole o permutandole. Come amministratore o operatore dei dati, come ingegnere dello storage, devi gestire e mantenere i dati in modo sicuro durante tutto il loro ciclo di vita. https://www.netapp.com/pdf.html?item=/media/19767-ds-3898-netapp-storage-encryption-volume-encryption.pdf["Crittografia dello storage NetApp (NSE) e amp;#44; crittografia dei volumi NetApp (NVE) e amp;#44; crittografia aggregata di NetApp"^] ti aiutano a crittografare tutti i tuoi dati a riposo in ogni momento, che siano o meno dati sensibili, e senza influire sulle operazioni quotidiane. https://www.netapp.com/pdf.html?item=/media/7563-ds-3213-en.pdf["NSE"^] è una soluzione hardware link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/index.html["dati a riposo"^] ONTAP che utilizza unità autocrittografanti convalidate FIPS 140-2 livello 2. https://www.netapp.com/pdf.html?item=/media/17070-ds-3899.pdf["NVE e NAE"^] sono una soluzione software link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/index.html["dati a riposo"^] ONTAP che utilizza  https://csrc.nist.gov/projects/cryptographic-module-validation-program/certificate/4144["Modulo crittografico NetApp validato FIPS 140-2 livello 1"^]. Con NVE e NAE, puoi usare sia dischi rigidi che unità a stato solido per la crittografia dei dati a riposo. Inoltre, le unità NSE possono essere usate per offrire una soluzione di crittografia nativa a più livelli che garantisce ridondanza e maggiore sicurezza. Se un livello viene violato, il secondo livello protegge comunque i dati. Queste funzionalità rendono ONTAP ben posizionato per  https://www.netapp.com/pdf.html?item=/media/17002-sb-3952.pdf["crittografia quantum-ready"^].
